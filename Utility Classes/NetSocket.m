...
- (void)_cfsocketSslFailed;
- (void)_cfsocketSslConnected;
...
- (BOOL)open
{
	...
	if( ![self _cfsocketCreated] )
	{
        ...
        setsockopt(nativeSocket,SOL_SOCKET,SO_KEEPALIVE,&sockVal,sizeof(int));
...
{
	[self writeData:[inString dataUsingEncoding:inEncoding allowLossyConversion:YES]];
}
...
- (void) startEncrypting
{
    ...
    SSL_CTX_set_verify(mSslCtx,SSL_VERIFY_NONE, nil);
    mSslItem = SSL_new(mSslCtx);
    SSL_set_connect_state(mSslItem);
    SSL_set_fd(mSslItem,[self nativeSocketHandle]);
    
    mIsHandshaking = YES;
    
    int rc = SSL_connect(mSslItem);
    if (rc == 1) {
        [self _cfsocketSslConnected];
    }
    else {
        int sslRc = SSL_get_error(mSslItem,rc);
        if ((sslRc != SSL_ERROR_WANT_READ) && (sslRc != SSL_ERROR_WANT_WRITE)) {
            [self _cfsocketSslFailed];
        }
        else {
            while ((sslRc == SSL_ERROR_WANT_READ) || (sslRc == SSL_ERROR_WANT_WRITE)) {
                usleep(50000);
                rc = SSL_connect(mSslItem);
                if (rc == 1) {
                    sslRc = 0;
                    [self _cfsocketSslConnected];
                }
                else {
                    sslRc = SSL_get_error(mSslItem,rc);
                }
            }
            if (sslRc != 0) {
                if (sslRc == SSL_ERROR_SSL) {
					char errorstr[160];

					ERR_error_string(sslRc,&errorstr[0]);
                    NSLog([NSString stringWithFormat:@"%s", errorstr]);
                }
                [self _cfsocketSslFailed];
            }
        }
...
- (void)_cfsocketSslConnected
{
	...
	// Notify our delegate that the socket has connected successfully
	if( [mDelegate respondsToSelector:@selector( netsocketSSLConnected: )] )
		[mDelegate netsocketSSLConnected:self];
...
- (void)_cfsocketSslFailed
{
    ...
    if ( [mDelegate respondsToSelector:@selector(netsocketSSLFailure:)] )
        [mDelegate netsocketSSLFailure:self];
}
...
- (void)_cfsocketDataAvailable
{
	...
	// We disconnected, if there was no data added to our incoming data buffer
...
- (void)_socketReadData
{
    ...
    if ([self isEncrypted]) {
        amountRead = SSL_read(mSslItem,readBuffer,amountAvailable);
    }
    else {    
	...
	{
        if([self isEncrypted]) {
            int sslRc = SSL_get_error(mSslItem,amountRead);
            if ((sslRc != SSL_ERROR_WANT_READ) && (sslRc != SSL_ERROR_WANT_WRITE)) {
                [self _cfsocketDisconnected];
            }
            else if (sslRc == SSL_ERROR_WANT_WRITE) {
                CFSocketEnableCallBacks([self cfsocketRef],kCFSocketWriteCallBack);
            }
        }
...
- (void)_socketWriteData
{
	...
	// Send all we can
    if ([self isEncrypted]) {
        amountSent = SSL_write(mSslItem,[mOutgoingBuffer bytes],[mOutgoingBuffer length]);
    }
    else {
	...
	{
        if([self isEncrypted]) {
            int sslRc = SSL_get_error(mSslItem,amountSent);
            if ((sslRc != SSL_ERROR_WANT_READ) && (sslRc != SSL_ERROR_WANT_WRITE)) {
